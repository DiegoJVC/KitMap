package com.cobelpvp.hcfactions.server.uhc;
...
public class UHCListener implements Listener {
    ...
    public void onPlayerJoin(PlayerJoinEvent event) {
        Bukkit.getScheduler().runTaskLater(HCFactions.getInstance(), () -> init(event.getPlayer()), 1L);
    }
    ...
    public void onRegen(EntityRegainHealthEvent event) {
        ...
        Faction faction = LandBoard.getInstance().getTeam(player.getLocation());

        if (faction == null || !faction.isMember(player.getUniqueId()) || SpawnTagHandler.isTagged(player)) {
            event.setCancelled(true);
        }
    }
    ...
    public void onPlayerDeath(PlayerDeathEvent event) {
        ...
        Bukkit.getScheduler().runTaskLater(HCFactions.getInstance(), () -> deathTime.remove(event.getEntity().getUniqueId()), 20L);
    }
