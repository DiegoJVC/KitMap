...
public class Faction {
    ...
    public void rename(String newName) {
        ...
        Atheneum.getInstance().runRedisCommand(redis -> {
            redis.del("kitmap_teams." + oldName.toLowerCase());
            return (null);
        });
    ...
    public void recalculatePoints() {
        ...
        basePoints += kothCaptures * 30;
        basePoints += (diamondsMined / 500) * 5;
    ...
    private String offlineMemberList(Player player, Collection<UUID> uuids) {
        ...
        uuids.forEach(uuid -> online.add((HCFactions.getInstance().getDeathbanMap().isDeathbanned(uuid) ? ChatColor.DARK_RED : isMember(player.getUniqueId()) ? ChatColor.RED : ChatColor.RED) + (isOwner(uuid) ? "***" : isCoLeader(uuid) ? "**" : isCaptain(uuid) ? "*" : "") + Bukkit.getOfflinePlayer(uuid).getName() + ChatColor.GRAY + '[' + (isMember(uuid) ? ChatColor.GREEN : ChatColor.RED) + HCFactions.getInstance().getKillsMap().getKills(uuid) + ChatColor.GRAY + ']'));
    ...
    private String memberList(Player player, Collection<UUID> uuids) {
        ...
        uuids.forEach(uuid -> online.add((HCFactions.getInstance().getDeathbanMap().isDeathbanned(uuid) ? ChatColor.DARK_RED : isMember(player.getUniqueId()) ? ChatColor.DARK_GREEN : ChatColor.RED) + (isOwner(uuid) ? "***" : isCoLeader(uuid) ? "**" : isCaptain(uuid) ? "*" : "") + Bukkit.getOfflinePlayer(uuid).getName() + ChatColor.GRAY + '[' + (isMember(uuid) ? ChatColor.GREEN : ChatColor.RED) + HCFactions.getInstance().getKillsMap().getKills(uuid) + ChatColor.GRAY + ']'));
