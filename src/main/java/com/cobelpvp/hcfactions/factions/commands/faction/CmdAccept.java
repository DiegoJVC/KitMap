...
public class CmdAccept {
    ...
    public static void teamAccept(Player sender, @Param(name = "team") Faction faction) {
        if (faction.getInvitations().contains(sender.getUniqueId())) {
            ...
            if (DTRHandler.isOnCooldown(faction) && !HCFactions.getInstance().getServerHandler().isPreEOTW() && !HCFactions.getInstance().getMapHandler().isKitMap() && !HCFactions.getInstance().getServerHandler().isCobelKitMap()) {
