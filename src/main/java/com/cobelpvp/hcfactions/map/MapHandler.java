...
import com.cobelpvp.hcfactions.map.stats.StatsHandler;
...
import com.cobelpvp.hcfactions.map.kits.KitManager;
...
public class MapHandler {
    ...
    public void load() {
        ...
        if (isKitMap() || HCFactions.getInstance().getServerHandler().isCobelKitMap()) {
            statsHandler = new StatsHandler();
            kitManager = new KitManager();

            Bukkit.getScheduler().runTaskLater(HCFactions.getInstance(), () -> {
                EventHandler kothHandler = HCFactions.getInstance().getEventHandler();
                ...
                KOTH selected = koths.get(Atheneum.RANDOM.nextInt(koths.size()));
                selected.activate();
            }, 5 * 60 * 20);
    ...
    private BasicDBObject getDefaults() {
        ...
        dbObject.put("teamSize", 15);
        dbObject.put("regenTimeDeath", 25);
        dbObject.put("regenTimeRaidable", 30);
        dbObject.put("scoreboardTitle", "&6CobelPvP &a[Map Alpha]");
        dbObject.put("mapStartedString", "Map Alpha - Started December 03, 2021");
        dbObject.put("warzone", 1000);
        dbObject.put("netherBuffer", 150);
        dbObject.put("worldBuffer", 300);
        dbObject.put("endPortalLocation", "1500, 1500");
