...
public class PvPTimerMap extends PersistMap<Integer> {
    ...
    public int getSecondsRemaining(UUID check) {
        if (HCFactions.getInstance().getServerHandler().isPreEOTW() || HCFactions.getInstance().getMapHandler().isKitMap() || HCFactions.getInstance().getServerHandler().isCobelKitMap()) {
            return (0);
        }
