...
public class HCFactionsScoreGetter implements ScoreGetter {
    ...
    public void getScores(LinkedList<String> scores, Player player) {
        ...
        if (HCFactions.getInstance().getMapHandler().isKitMap() || HCFactions.getInstance().getServerHandler().isCobelKitMap()) {
            StatsEntry stats = HCFactions.getInstance().getMapHandler().getStatsHandler().getStats(player.getUniqueId());
            scores.add("&4&lKills: &c" + stats.getKills());
            scores.add("&4&lDeaths: &c" + stats.getDeaths());
        }
