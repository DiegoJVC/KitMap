...
public class DammyFixCommand {
    ...
    @Command(names = {"damyidiot_fuck"}, permission = "op")
    public static void fixKitmap(CommandSender sender) {
        DBCollection coll = HCFactions.getInstance().getMongoPool().getDB(HCFactions.MONGO_DB_NAME).getCollection("Teams");
        DBCursor cursor = coll.find();

        cursor.forEach(document -> {
            if (!document.containsKey("Name")) return;

            Faction faction = new Faction((String) document.get("Name"));
            faction.load((BasicDBObject) document);

            HCFactions.getInstance().getFactionHandler().setupTeam(faction);
        });
    }
