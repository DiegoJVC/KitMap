...
public class CombatLoggerListener implements Listener {
    ...
    public void onEntityDeath(EntityDeathEvent event) {
        if (event.getEntity().hasMetadata(COMBAT_LOGGER_METADATA)) {
        		boolean isKitMap = HCFactions.getInstance().getMapHandler().isKitMap() || HCFactions.getInstance().getServerHandler().isCobelKitMap();
            ...
            if (isKitMap) {
                HCFactions.getInstance().getMapHandler().getStatsHandler().getStats(metadata.playerUUID).addDeath();
            }

            int victimKills = HCFactions.getInstance().getMapHandler().isKitMap() || HCFactions.getInstance().getServerHandler().isCobelKitMap() ? HCFactions.getInstance().getMapHandler().getStatsHandler().getStats(event.getEntity().getUniqueId()).getKills() : HCFactions.getInstance().getKillsMap().getKills(event.getEntity().getUniqueId());
            ...
            if (event.getEntity().getKiller() != null) {
                ...
                if (isKitMap) {
                    HCFactions.getInstance().getMapHandler().getStatsHandler().getStats(event.getEntity().getKiller()).addKill();

                    killerKills = HCFactions.getInstance().getMapHandler().getStatsHandler().getStats(event.getEntity().getKiller()).getKills();
                }
                ...
                String deathMessage = ChatColor.RED + metadata.playerName + ChatColor.GOLD + "[" + victimKills + "]" + ChatColor.GRAY + " (Combat-Logger)" + ChatColor.YELLOW + randomMessage + ChatColor.RED + event.getEntity().getKiller().getName() + ChatColor.GOLD + "[" + killerKills + "]" + ChatColor.YELLOW + ".";

                for (Player player : Bukkit.getOnlinePlayers()) {
                    if (HCFactions.getInstance().getToggleDeathMessageMap().areDeathMessagesEnabled(player.getUniqueId())) {
                ...
                String deathMessage = ChatColor.RED + metadata.playerName + ChatColor.GOLD + "[" + victimKills + "]" + ChatColor.GRAY + " (Combat-Logger)" + ChatColor.YELLOW + " died.";
                ...
                new BukkitRunnable() {
                    ...
                    public void run() {
                        ...
                        if (HCFactions.getInstance().getMapHandler().isKitMap() || HCFactions.getInstance().getServerHandler().isCobelKitMap()) {
                            victimKills = HCFactions.getInstance().getMapHandler().getStatsHandler().getStats(metadata.playerUUID).getKills();
                        }
